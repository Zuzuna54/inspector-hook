...
const SessionsView = {
  ...
  getSessionDisplayName(session, includeBranch = false) {
    ...
    // 3. Fall back to truncated session ID
    else {
    ...
    if (includeBranch && session.metadata?.gitBranch) {
      return `${name} (${session.metadata.gitBranch})`;
    }
  ...
  init() {
    ...
    this._unsubscribers.push(
      State.subscribe('sessionView', (newVal, oldVal) => {
        ...
        if (newVal.selectedSession !== oldVal?.selectedSession ||
  ...
  getFilteredSessions() {
    ...
    if (searchQuery) {
      filtered = sessions.filter(session => {
        ...
        const workingDir = (session.metadata?.workingDirectory || '').toLowerCase();
  ...
  countErrors(session) {
    ...
    return session.toolExecutions.filter(t =>
      ...
      return `
        <div class="sv-card ${statusClass} ${isSelected ? 'selected' : ''} ${hasErrors ? 'has-errors' : ''}"
          ...
          <div class="sv-card-header">
            ...
            <span class="sv-card-name" title="${Utils.escapeHtml(session.id)}">${Utils.escapeHtml(displayName)}</span>
  ...
  renderActivityItem(activity, idx) {
    ...
    switch (activity.type) {
      ...
      case 'notification':
        ...
        const notifIcon = notifType === 'permission_prompt' ? '🔐' :
                         notifType === 'idle_prompt' ? '⏸️' : '🔔';
        return `
      ...
      case 'message':
        ...
        const levelClass = msgData.level === 'error' ? 'sv-message-error' :
                          msgData.level === 'warn' ? 'sv-message-warn' : '';
        return `
  ...
  renderToolDetails(tool) {
    const inputStr = tool.input ?
      (typeof tool.input === 'object' ? JSON.stringify(tool.input, null, 2) : String(tool.input)) : '';
  ...
  getToolType(toolName) {
    ...
    if (name.includes('glob') || name.includes('grep') || name.includes('search')) return 'search';
  ...
  confirmDeleteSession(sessionId) {
    ...
    const sessionName = session ? this.getSessionDisplayName(session) : sessionId.slice(0, 8);
